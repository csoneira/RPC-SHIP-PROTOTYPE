...
% Combine masks to evaluate efficiencies for detector cross-checks
thin_pair_exists_mask = thinTop_exists_Mask & thinBot_exists_Mask;
thin_pair_range_mask  = thinTop_range_Mask & thinBot_range_Mask;
mask_cases = {
    % Thin top – existence
    'ThinTop',  'existence', 'PMT exists',                          pmt_exists_Mask,                                      thinTop_exists_Mask;
    'ThinTop',  'existence', 'PMT & Thick exist',                   (pmt_exists_Mask & thick_exists_Mask),                thinTop_exists_Mask;
    'ThinTop',  'existence', 'PMT & ThinBot exist',                 (pmt_exists_Mask & thinBot_exists_Mask),              thinTop_exists_Mask;
    'ThinTop',  'existence', 'PMT & Thick & ThinBot exist',         (pmt_exists_Mask & thick_exists_Mask & thinBot_exists_Mask), thinTop_exists_Mask;
    % Thin top – range
    'ThinTop',  'range',     'PMT in range',                        pmt_range_Mask,                                       thinTop_range_Mask;
    'ThinTop',  'range',     'PMT & Thick in range',                (pmt_range_Mask & thick_range_Mask),                  thinTop_range_Mask;
    'ThinTop',  'range',     'PMT & ThinBot in range',              (pmt_range_Mask & thinBot_range_Mask),                thinTop_range_Mask;
    'ThinTop',  'range',     'PMT & Thick & ThinBot in range',      (pmt_range_Mask & thick_range_Mask & thinBot_range_Mask), thinTop_range_Mask;
    % Thin bottom – existence
    'ThinBot',  'existence', 'PMT exists',                          pmt_exists_Mask,                                      thinBot_exists_Mask;
    'ThinBot',  'existence', 'PMT & Thick exist',                   (pmt_exists_Mask & thick_exists_Mask),                thinBot_exists_Mask;
    'ThinBot',  'existence', 'PMT & ThinTop exist',                 (pmt_exists_Mask & thinTop_exists_Mask),              thinBot_exists_Mask;
    'ThinBot',  'existence', 'PMT & Thick & ThinTop exist',         (pmt_exists_Mask & thick_exists_Mask & thinTop_exists_Mask), thinBot_exists_Mask;
    % Thin bottom – range
    'ThinBot',  'range',     'PMT in range',                        pmt_range_Mask,                                       thinBot_range_Mask;
    'ThinBot',  'range',     'PMT & Thick in range',                (pmt_range_Mask & thick_range_Mask),                  thinBot_range_Mask;
    'ThinBot',  'range',     'PMT & ThinTop in range',              (pmt_range_Mask & thinTop_range_Mask),                thinBot_range_Mask;
    'ThinBot',  'range',     'PMT & Thick & ThinTop in range',      (pmt_range_Mask & thick_range_Mask & thinTop_range_Mask), thinBot_range_Mask;
    % Thick RPC – existence
    'Thick',    'existence', 'PMT exists',                          pmt_exists_Mask,                                      thick_exists_Mask;
    'Thick',    'existence', 'PMT & ThinTop exist',                 (pmt_exists_Mask & thinTop_exists_Mask),              thick_exists_Mask;
    'Thick',    'existence', 'PMT & ThinBot exist',                 (pmt_exists_Mask & thinBot_exists_Mask),              thick_exists_Mask;
    'Thick',    'existence', 'PMT & ThinTop & ThinBot exist',       (pmt_exists_Mask & thin_pair_exists_mask),            thick_exists_Mask;
    % Thick RPC – range
    'Thick',    'range',     'PMT in range',                        pmt_range_Mask,                                       thick_range_Mask;
    'Thick',    'range',     'PMT & ThinTop in range',              (pmt_range_Mask & thinTop_range_Mask),                thick_range_Mask;
    'Thick',    'range',     'PMT & ThinBot in range',              (pmt_range_Mask & thinBot_range_Mask),                thick_range_Mask;
    'Thick',    'range',     'PMT & ThinTop & ThinBot in range',    (pmt_range_Mask & thin_pair_range_mask),              thick_range_Mask;
    };
numMaskCases = size(mask_cases,1);
detector_col  = strings(numMaskCases,1);
mask_type_col = strings(numMaskCases,1);
condition_col = strings(numMaskCases,1);
passing_col   = zeros(numMaskCases,1);
detected_col  = zeros(numMaskCases,1);
eff_col       = nan(numMaskCases,1);
unc_col       = nan(numMaskCases,1);
for idx = 1:numMaskCases
    detector_col(idx)  = string(mask_cases{idx,1});
    mask_type_col(idx) = string(mask_cases{idx,2});
    condition_col(idx) = string(mask_cases{idx,3});
    denom_mask  = logical(mask_cases{idx,4});
    target_mask = logical(mask_cases{idx,5});
    passing = sum(denom_mask);
    detected = sum(denom_mask & target_mask);
    passing_col(idx)  = passing;
    detected_col(idx) = detected;
    if passing > 0
        eff = detected / passing;
        eff_col(idx) = eff;
        unc_col(idx) = sqrt(eff*(1-eff)/passing);
    end
end
mask_efficiency_summary = table(detector_col, mask_type_col, condition_col, ...
    passing_col, detected_col, round(eff_col,4), round(unc_col,4), ...
    'VariableNames', {'detector','mask_type','condition','passing_events','detected_events','eff','eff_uncertainty'});
disp(mask_efficiency_summary);
csv_filename = sprintf('efficiency_summary_run_%d.csv', run);
writetable(mask_efficiency_summary, csv_filename);
mask_csv_filename = sprintf('efficiency_mask_summary_run_%d.csv', run);
if ~strcmp(mask_csv_filename, csv_filename)
    writetable(mask_efficiency_summary, mask_csv_filename);
end
return;